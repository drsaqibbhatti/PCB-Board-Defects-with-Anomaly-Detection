...
#     def __init__(self, in_ch, out_ch):
#         super().__init__()
#         self.fc1 = torch.nn.Sequential(torch.nn.Linear(in_ch, out_ch))
        
#         self.bn = torch.nn.BatchNorm2d(out_ch)
#         self.fc2 = torch.nn.Sequential(torch.nn.SiLU(inplace=True),
...
checkpoint_path= "/mnt/d/hvs/Hyvsion_Projects/Pre_training/deeplearningresearch/python/project/simplenet/EfficientADTeacher.pth"
# Load raw state dict
...
model_g = Generator(in_ch=multi_input_channels, out_ch=feature_size)
...
for epoch in range(1, epochs):
    ...
    for input in trainLoader:
        ...
        loss, trueLoss, fakeLoss, truescore, fakescore = criterion(multi_feat, model_g, model_d)
    ...
    with torch.no_grad():
        for input in trainLoader:  # or a separate good-only val set
            ...
            score = -model_d(model_g(multi_feat))
    ...
    threshold = np.percentile(normal_scores, 95)  # 95% of normal images fall below this
    ...
    with torch.no_grad():
        ...
        for input in evalLoader:
            ...
            # Compute scores
            scores = -model_d(model_g(multi_feat)) 
