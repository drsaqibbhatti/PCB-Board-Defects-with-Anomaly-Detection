...
def compute_metrics(outputs, targets):
    ...
    f1 = metrics.f1_score(targets, (outputs > 0.5).astype('int32'))
...
def get_latest_run_path(base_dir="./trained_models/SimpleNet/SimpleNetTrainerV1"):
    runs = [d for d in os.listdir(base_dir) if d.startswith("run_")]
    if not runs:
        raise FileNotFoundError("No run folders found in path.")
    latest_run = sorted(runs, key=lambda x: int(x.split("_")[1]))[-1]
    return os.path.join(base_dir, latest_run)


def plot_and_save_training_logs(log_path):
    ...
    metrics = ['Train Loss', 'F1', 'ACC', 'ROC_AUC', 'AUC']

    for metric in metrics:
        plt.figure()
        plt.plot(log['Epoch'], log[metric], marker='o')
        plt.title(f'{metric} over Epochs')
        plt.xlabel('Epoch')
        plt.ylabel(metric)
        plt.grid(True)
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, f"{metric.replace(' ', '_').lower()}.png"))
        plt.close()
...
# ✅ Automatically find latest log
try:
    latest_run_path = get_latest_run_path()
    log_path = os.path.join(latest_run_path, 'training_log.csv')
    if os.path.exists(log_path):
        saved_images = plot_and_save_training_logs(log_path)
    else:
        saved_images = ["Log file not found"]
